...
class wfModel(nn.Module):
    ...
    def forward(self, inputs, rnn_state):
        ...
        rnn_output = self.rnn.project_lorentz_manual(rnn_output)   
        rnn_output_eucl = hc_math.logmap0(rnn_output,  k=self.k_tensor, is_tan_normalize=False)
        output = self.dense(rnn_output_eucl)
        return output, rnn_state

class LorentzRNNwavefunction(object):
    def __init__(self,systemsize_x, systemsize_y, _units,  _num_in, spatial_clamp, non_lin, seed):
        ...
        self.name = f'Lorentz2dRNN_u={self.units}'

        self.rnn=Lorentz2dRNNcell(self.units, self.num_in, self.spatial_clamp, self.non_lin, k=1.0)
    ...
    def sample(self, numsamples):
        ...
        # --- Boundary Conditions ---
        # Instead of zeros, use the manifold's origin point for rnn_states (often called the 'pole')
        # For Lorentz, this is usually (sqrt(k), 0, 0, ...)
        for ny in range(self.Ny):
            nx_bound = -1 if ny % 2 == 0 else self.Nx
            rnn_states[f"{nx_bound}{ny}"] = hc_math.expmap0(torch.zeros((numsamples, self.units), dtype=self.dtype, device=device), k=self.k_tensor)
            inputs[f"{nx_bound}{ny}"] = torch.zeros((numsamples, self.inputdim), dtype=self.dtype, device=device)

        for nx in range(self.Nx):
            rnn_states[f"{nx}-1"] =  hc_math.expmap0(torch.zeros((numsamples, self.units), dtype=self.dtype, device=device), k=self.k_tensor)
    ...
    def log_probability(self, samples):
        ...
        for ny in range(self.Ny):
            nx_bound = -1 if ny % 2 == 0 else self.Nx
            rnn_states[f"{nx_bound}{ny}"] = hc_math.expmap0(torch.zeros((self.numsamples, self.units), dtype=self.dtype, device=device), k=self.k_tensor)
            inputs[f"{nx_bound}{ny}"] = torch.zeros((self.numsamples, self.inputdim), dtype=self.dtype, device=device)

        for nx in range(self.Nx):
            rnn_states[f"{nx}-1"] = hc_math.expmap0(torch.zeros((self.numsamples, self.units), dtype=self.dtype, device=device), k=self.k_tensor)
