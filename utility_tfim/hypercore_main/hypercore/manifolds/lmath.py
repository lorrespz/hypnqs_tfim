...
def _dist_original(x, y, k: torch.Tensor, keepdim: bool = False, dim: int = -1):
    d = -_inner(x, y, dim=dim, keepdim=keepdim)
    return acosh(d / k)

# Rewritten version on March 20, 2026
def _dist(x, y, k: torch.Tensor, keepdim: bool = False, dim: int = -1, eps: float = 1e-7):
    # Calculate the Lorentz inner product scaled by curvature
    # We want -k * <x, y>_L to be >= 1.0
    inner_prod = -_inner(x, y, dim=dim, keepdim=keepdim)
    
    # Standard: inner_prod / k
    # Stable: Clamp it so it's at least 1 + eps
    res = torch.clamp(inner_prod / k, min=1.0 + eps)    
    return torch.acosh(res)
...
def _dist0_original(x, k: torch.Tensor, keepdim: bool = False, dim: int = -1):
    d = -_inner0(x, k=k, dim=dim, keepdim=keepdim)
    return acosh(d / k)

#Rewritten version on March 20, 2026
def _dist0(x, k: torch.Tensor, keepdim: bool = False, dim: int = -1, eps: float = 1e-7):
    # _inner0 usually calculates the Lorentz inner product 
    # between x and the origin [sqrt(1/k), 0, 0...]
    d = -_inner0(x, k=k, dim=dim, keepdim=keepdim)
    
    # Stability: ensure d/k is >= 1.0 + eps
    # This prevents the gradient from exploding when x is at the origin
    res = torch.clamp(d / k, min=1.0 + eps)
...
#Original version
def _expmap_original(x, u, k: torch.Tensor, dim: int = -1):
...
#Rewritten version on March 20, 2026
def _expmap(x, u, k: torch.Tensor, dim: int = -1, eps: float = 1e-10):
    nomin = _norm(u, keepdim=True, dim=dim)
    
    # Safe normalization: only divide if norm > 0
    u_normalized = torch.where(nomin > eps, u / (nomin + eps), torch.zeros_like(u))
    
    nomin_clamped = nomin.clamp_max(EXP_MAX_NORM)
    sqrt_c = torch.sqrt(k)
    
    # The actual map
    p = torch.cosh(nomin_clamped / sqrt_c) * x + \
        torch.sinh(nomin_clamped / sqrt_c) * u_normalized * sqrt_c
    return p
...
#Original version
def _expmap0_original(u, k: torch.Tensor, dim: int = -1, norm_control = False, eps: float = 1e-10):
...
#Rewritten version on March 20, 2026
def _expmap0(u, k: torch.Tensor, dim: int = -1, norm_control = False, eps: float = 1e-10):
    norm_u = _norm(u, keepdim=True, dim=dim).clamp_max(50)
    
    if norm_control:
        denom = (norm_u / torch.sqrt(k)).clamp_max(1)
    else:
        denom = (norm_u / torch.sqrt(k))
        
    sqrt_k = torch.sqrt(k)
    l_v = torch.cosh(denom) * sqrt_k
    
    # Use the limit sinh(x)/x -> 1 as x -> 0 for the right term
    # This is much more stable than (sinh(norm)/norm) * u
    sync_factor = torch.where(
        norm_u > eps, 
        torch.sinh(denom) / (norm_u + eps), 
        1.0 / sqrt_k # Limit value
    )
...
def _logmap_original(x, y, k, dim: int = -1, is_tan_normalize = False):
...
#Rewritten _logmap on March 20, 2026
def _logmap(x, y, k, dim: int = -1, is_tan_normalize = False, eps=1e-7):
    dist_ = _dist(x, y, k=k, dim=dim, keepdim=True)
    ...
    # STABILIZATION: 
    # If the points are too close, the tangent vector is effectively zero.
    # We use torch.where to avoid dividing by a tiny denom.
    y_tan = torch.where(
        dist_ > eps,
        dist_ * nomin / (denom + eps), # Add eps to denom or use where
        torch.zeros_like(nomin)
    )
...
#original version
def _logmap0_original(y, k, dim: int = -1 , is_tan_normalize = False):
...
def _logmap0(y, k, dim: int = -1, is_tan_normalize=False, eps: float = 1e-7):
    # 1. Distance to origin
    dist_ = _dist0(y, k=k, dim=dim, keepdim=True)
    
    # 2. Construct the tangent projection (nomin)
    nomin_ = 1.0 / k * _inner0(y, k=k, keepdim=True, dim=dim) * torch.sqrt(k)
    dn = y.size(dim) - 1
    
    # Assemble the projection vector
    nomin = torch.cat((nomin_ + y.narrow(dim, 0, 1), y.narrow(dim, 1, dn)), dim)
    denom = _norm(nomin, keepdim=True, dim=dim)
    
    # 3. Safe Division
    # If the point is at the origin, the logmap is simply a zero vector.
    # We use a threshold (eps) to prevent 0/0.
    ...
    res = dist_ * direction    
    if is_tan_normalize:
        # Optional: ensure the tangent vector is scaled correctly for the manifold
        res = normalize_tangent(res, k)        
    return res
