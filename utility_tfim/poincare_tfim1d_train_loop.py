...
from poincare_tfim1d_wf import *
from poincare_rsgd import RSGD
...
def Ising1D_local_energies(Jz, Bx, N, samples, wf, lp_s=None):
    # If samples is a PyTorch tensor, make sure we handle it natively
    numsamples = samples.shape[0]

    # Move Jz to a tensor if it's currently a numpy array
    if isinstance(Jz, np.ndarray):
        Jz_tensor = torch.from_numpy(Jz).to(samples.device)
    else:
        Jz_tensor = Jz.to(samples.device)

    # --- 1. Efficient PyTorch Vectorized Diagonal Energies ---
    spins_physical = 1 - 2 * samples  # Performs operation directly on GPU/CPU tensor

    # Multiply neighboring spins
    diagonal_terms = spins_physical[:, :-1] * spins_physical[:, 1:]

    # Use torch.sum along dim=1 instead of np.sum along axis=1
    local_energies = -torch.sum(diagonal_terms * Jz_tensor[:N-1], dim=1)

    # --- 2. Non-Diagonal Elements (Transverse Field Bx) ---
    if Bx != 0:
        # Clone and repeat natively in PyTorch
        flip_queue = samples.unsqueeze(0).repeat(N, 1, 1)

        # Fast diagonal masking in PyTorch
        idx = torch.arange(N, device=samples.device)
        flip_queue[idx, :, idx] = 1 - flip_queue[idx, :, idx]

        flip_queue_reshaped = flip_queue.view(N * numsamples, N)

        # --- 3. Compute Log Probs of Flips in Batches ---
        lp_flips_list = []
        for i in range(0, N * numsamples, numsamples):
            batch_tensor = flip_queue_reshaped[i : i + numsamples]
            with torch.no_grad():
        ...
        if lp_s is None:
            ...
            # Keep it as a tensor
            lp_orig = lp_s if torch.is_tensor(lp_s) else torch.tensor(lp_s, device=samples.device)

        # Accumulate off-diagonal exponential expectations using torch math
        local_energies += -Bx * torch.sum(torch.exp(0.5 * lp_flips - 0.5 * lp_orig), dim=0)
...
def run_1DTFIM(wf, numsteps=1001, N=100, Bx=+1, numsamples=100,
    ...
    for step in range(numsteps):
        ...
        np.save(f'{fname}/{wf.name}_N={N}_ns={numsamples}_rmax={wf.r_max}_{wf.h_non_lin}_meanE.npy', meanEnergy)
        np.save(f'{fname}/{wf.name}_{N}_ns={numsamples}_rmax={wf.r_max}_{wf.h_non_lin}_varE.npy', varEnergy)
        ...
        if step == 0:
            ...
            best_E_list.append(meanE)
            torch.save(wfmodel.state_dict(), f'{fname}/{wf.name}_N={N}_ns={numsamples}_rmax={wf.r_max}_{wf.h_non_lin}_checkpoint.pt')
        ...
        if step % 10 == 0:
            ...
            if hyp_vars:
                ...
                np.save(f'{fname}/{wf.name}_N={N}_rmax={wf.r_max}_ns={numsamples}_{wf.h_non_lin}_rmax.npy',r_val_list)
